...
class mylora(LoRa):
    ...
    async def start(self):
        ...
        '''
        t = Timer(300,non_active,args=([9999],))
        t.start()
        '''
        o_img = -99
        o_x = -99
        ...
        while True:
            ...
            if board == Board3:
                ...
                '''
                img_id1 = int(mode(payload[4],payload[8],payload[12]))
                img_id2 = int(mode(payload[5],payload[9],payload[13]))
                img_id = img_id1*256 + img_id2
                x = int(list(payload[6:7])[0])
                y = int(list(payload[7:8])[0])
                x1 = int(list(payload[10:11])[0])
                y1 = int(list(payload[11:12])[0])
                x2 = int(list(payload[14:15])[0])
                y2 = int(list(payload[15:16])[0])
                x = int(mode(x,x1,x2))
                y = int(mode(y,y1,y2))
                print(f'ox  {o_img} {o_x}_{o_y} ')
                print(f'nx  {img_id} {x}_{y} ')
                '''
                ...
                #if the packet send and recieve successfully
                '''
                if o_img == img_id and (o_x != x or o_y != y):
                    print('==============hhhhhhhhhhhhh==============---------')
                    os.rename(f'{SOURCE_DIR}{img_id}/{x}_{y}.jpg', f'{EXPORTED_DIR}{img_id}/{x}_{y}.jpg')
                o_img = img_id
                o_x = x
                o_y = y
                '''
                if os.path.isdir(f'./lora_receiver/rx_buffer/{img_id}/{x}_{y}/'):
                    os.rmdir(f'./lora_receiver/rx_buffer/{img_id}/{x}_{y}/')
                ...
                t = Timer(30,non_active,args=([img_id],))
...
def non_active(img_id):
    print(" again sending")
    img_id = img_id[0]
    allpicrecieve=os.listdir(f'./lora_receiver/rx_buffer/{img_id}')
    for x in range (0,len(allpicrecieve)//6):
        for y in range(0,6):
            if len(os.listdir(f'./lora_receiver/rx_buffer/{img_id}/{x}_{y}') )!= 0:
                os.rename(f'./image_buffer/exported/000{img_id}/{x}_{y}.jpg',f'./image_buffer/segmented/000{img_id}/{x}_{y}.jpg')
